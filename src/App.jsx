...
function App() {
  const [modo, setModo] = useState('cliente'); // cliente | login-dono | dono
  ...
  const [bloqueioBarbeiro, setBloqueioBarbeiro] = useState('todos'); // todos | id
  ...
  async function escolherData(data) {
    ...
    let qBloq = supabase.from('dias_bloqueados').select('barbeiro_id').eq('data', dataISO);
    const { data: bloqueios } = await qBloq;
    if (bloqueios && bloqueios.length > 0) {
      const barbeiroId = barbeiroEscolhido && !barbeiroEscolhido.semPref ? barbeiroEscolhido.id : null;
      // Fechado se: existe bloqueio da barbearia toda (barbeiro_id null), ou bloqueio do barbeiro escolhido
  ...
  // Agendar manual (o dono marca pra um cliente)
  ...
  return (
    <div style={estilos.tela}>
      <div style={{ textAlign: 'center', marginBottom: '24px' }}>
        <h1 style={{ color: OURO, margin: 0 }}>PRÎMEN</h1>
        <p style={{ color: '#8a8a8a', fontSize: '12px', letterSpacing: '1px' }}>BARBA · CABELO · BIGODE</p>
      </div>
